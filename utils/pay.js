...
/**
 * 支付
 * @param {*} goods_price 商品价格 数字类型
 * @param {*} order_id 订单ID
 * @param {*} goods_name 商品名称
 * @param {*} current_num 商品数量
 * @param {*} functionType 0 服务 1云门诊
 */
var pay =function pay(goods_price, order_id, goods_name, current_num, functionType) {
//如果支付金额为0 则不需要支付
  if(goods_price == 0){
...
goods_price = MoneyUtils.accMul(goods_price,100)  
  ...
  return new Promise(function (resolve, reject) {
    WXAPI.smallWxPay(postData).then(function (res) {
      if (res.code == 0) {

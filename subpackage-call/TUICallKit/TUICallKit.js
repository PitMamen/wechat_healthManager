...
Component({
    ...
    methods: {
        ...
        qryRightsUseRecord() {
            ...
            WXAPI.qryRightsUseRecord({ orderId: this.data.MyGroupID.replace('M_', '') }).then((res) => {
                if (res.code == 0 && res.data && res.data.length > 0 && res.data[0].leftServiceTime) {
                    this.data.allChatTimeNum = res.data[0].leftServiceTime*60
                    this.data.chatTimeNum = res.data[0].leftServiceTime*60
                }
        ...
        saveVideoNum() {
            ...
            WXAPI.saveVideoNum({ imGroupId: this.data.MyGroupID, usedTime: this.data.allChatTimeNum - this.data.chatTimeNum }).then((res) => {
                if (res.code == 0) {
                    console.log('保存使用时长成功')
                }

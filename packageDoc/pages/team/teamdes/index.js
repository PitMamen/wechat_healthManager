...
Page({
    ...
    onLoad: function (options) {
        console.log('team-des options', options)
        this.setData({
    ...
    onDocClick(event) {
        ...
        this.goDor(doc.doctorUserId, doc.doctorUserName)
    },

    onLeaderClick() {
        this.goDor(this.data.info.leaderPkgManageItem.doctorUserId, this.data.info.leaderPkgManageItem.doctorUserName)
    },

    goDor(id, name) {
        wx.navigateTo({
            url: `/packageDoc/pages/doctor/info/index?id=${id}&title=${name}`
        })
    },
    ...
    async switchHospital() {
        const res = await WXAPI.switchHospital({

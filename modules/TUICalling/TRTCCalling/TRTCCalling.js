...
class TRTCCalling {
  ...
  handleCallMode(callType, callModeMessage) {
    ...
    const enableCamera = callType === CALL_TYPE.VIDEO ? false : true
  ...
  setInviteIDList(inviteID) {
    if (this.data.invitation.inviteID === inviteID) {
  ...
  async groupCall(params) {
    const { type } = params
    // 生成房间号，拼接URL地址
    const roomID = this.data.roomID || Math.floor(Math.random() * 100000000 + 1) // 随机生成房间号
    this.enterRoom({ roomID, callType: type })
    try {
